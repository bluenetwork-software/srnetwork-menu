$(document).ready(function () {
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            buttons.length = 0
            submenus.length = 0
            $.post('http://srnetwork-menu/close', JSON.stringify({}));
    ...
    function goback() {
        var gobackbutton = (`<button style="height:10px" id="goback"class ="btn"><div class="title" style="margin-top:-3.1%">Geri Dön</div><i class="fas fa-chevron-left" style="float:right;margin-top:-3%"></i></button>`)
        $(".container").append(gobackbutton).fadeIn(400)
    }

    // Events
    $("body").on("click" , ".a" , function(){
        $.post('http://srnetwork-menu/clicked', JSON.stringify({trigger:$(this).attr("data-trigger") , param:$(this).attr("data-parm")}));
        $.post('http://srnetwork-menu/close', JSON.stringify({}));
        $(".container").fadeOut('slow')
        setTimeout(() => {
            $("button").remove()
